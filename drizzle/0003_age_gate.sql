...
-- ---------------------------------------------------------------------
-- 2. Carry existing verified adults forward. GUARDED.
--
...
--
--    adult_verified_at/at_by are added by 0002 — but a database that
--    never received 0002 (e.g. production, per D's 2026-08-03 inspection)
--    has no such columns and no data was ever lost there. Guarding keeps
--    this file runnable against any database, with or without 0002.
-- ---------------------------------------------------------------------
do $$
begin
    if exists (select 1 from information_schema.columns
       ...
       and exists (select 1 from information_schema.columns
                where table_schema = 'public' and table_name = 'members'
                      and column_name = 'adult_verified_by') then

        execute $c$comment on column members.adult_verified_at is
            'DEPRECATED as of 0003. age_status is authoritative. Retained so an '
            'in-flight deploy does not break; read member_is_adult() instead.'$c$;
